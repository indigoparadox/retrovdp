...
#if defined( RETROFLAT_API_SDL1 )
      CRT_PIX_FORMAT_RGBA, state->platform.buffer.surface->pixels );
#elif defined( RETROFLAT_API_SDL2 )
      CRT_PIX_FORMAT_RGBA, NULL );
#else
      /* TODO */
      CRT_PIX_FORMAT_BGRA, state->platform.buffer.bits );
...
MERROR_RETVAL retroflat_vdp_flip( struct RETROFLAT_STATE* state ) {
   ...
   assert( 4 == state->platform.buffer.surface->format->BytesPerPixel );
#endif /* RETROFLAT_API_SDL1 */
